import { s as sanitize_props, a as spread_props, c as slot, d as ensure_array_like, f as attr_class, g as attr, e as escape_html } from "./index.js";
...
import { B as Book_marked } from "./book-marked.js";
...
const settings = new Persisted("settings", { ...DEFAULT_SETTINGS });
new Persisted("bookmarks", []);
new Persisted("notes", {});
new Persisted("hls", {});
const read = new Persisted("read", []);
const readlog = new Persisted("readlog", {});
new Persisted("khatamToasted", false);
const tasbih = new Persisted("tasbih", { c: 0, t: 33, label: "Tasbih bebas" });
const city = new Persisted("city", 0);
const sgdays = new Persisted("sgdays", 30);
new Persisted("onboarded", false);
const mathuratState = new Persisted("mathurat", null);
new Persisted("mgquiz", { best: 0 });
const puasa = new Persisted("puasa", { adj: 0, recs: [] });
new Persisted("manasik", { t: 0, s: 0 });
...
const VKEY = { sughra: "s", kubra: "k" };
const itemsFor = (v) => MASTER.filter((it) => it.reps[VKEY[v]] != null).map((it) => ({
  ...it,
  reps: it.reps[VKEY[v]]
}));
const LISTS = {
...
const JENIS_LABEL = { quran: "Al-Quran", zikir: "Zikir Ma'thur", doa: "Doa" };
const BASMALAH = "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ";
const pickTime = (f, mode) => typeof f === "string" ? f : f[mode];
const pickText = (item, version, mode, key) => {
  const kField = item[key + "K"];
...
function SideNav($$renderer, $$props) {
  ...
  let moreOpen = false;
  const mainNav = [
    { id: "home", icon: House, label: "Home", href: "/hub" },
    {
      id: "mushaf",
      icon: Book_open,
  ...
  $$renderer.push(`<!--]--> <nav class="fixed left-4 top-1/2 z-50 flex -translate-y-1/2 flex-col items-center" aria-label="Main navigation"><div class="side-pill flex flex-col items-center gap-1 rounded-[2rem] px-2 py-3 svelte-12buoxc"><div class="logo-btn mb-2 flex h-10 w-10 items-center justify-center rounded-full svelte-12buoxc"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 2C8 2 4 6 4 10c0 5 8 12 8 12s8-7 8-12c0-4-4-8-8-8z" fill="currentColor" opacity="0.9"></path><path d="M12 6c-1.5 2-2 4-2 5.5C10 13.5 11 15 12 16c1-1 2-2.5 2-4.5C14 10 13.5 8 12 6z" fill="white" opacity="0.6"></path></svg></div> <!--[-->`);
  const each_array = ensure_array_like(mainNav);
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    let item = each_array[$$index];
    if (item.id === "mathurat") {
      $$renderer.push("<!--[0-->");
      $$renderer.push(`<button${attr_class("nav-item relative flex h-11 w-11 items-center justify-center rounded-full transition-all duration-200 svelte-12buoxc", void 0, { "active": active === item.id })}${attr("aria-label", item.label)}${attr("title", item.label)}>`);
      if (item.icon) {
        $$renderer.push("<!--[-->");
        item.icon($$renderer, { size: 20, strokeWidth: 1.8 });
        $$renderer.push("<!--]-->");
      } else {
        $$renderer.push("<!--[!-->");
        $$renderer.push("<!--]-->");
      }
      $$renderer.push(` <span class="tooltip pointer-events-none absolute left-14 whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-medium opacity-0 transition-opacity duration-150 svelte-12buoxc">${escape_html(item.label)}</span></button>`);
    } else {
      $$renderer.push("<!--[-1-->");
      $$renderer.push(`<a${attr("href", item.href)}${attr_class("nav-item relative flex h-11 w-11 items-center justify-center rounded-full transition-all duration-200 svelte-12buoxc", void 0, { "active": active === item.id })}${attr("aria-label", item.label)}${attr("title", item.label)}>`);
      if (item.icon) {
        $$renderer.push("<!--[-->");
        item.icon($$renderer, { size: 20, strokeWidth: 1.8 });
        $$renderer.push("<!--]-->");
      ...
      $$renderer.push(` <span class="tooltip pointer-events-none absolute left-14 whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-medium opacity-0 transition-opacity duration-150 svelte-12buoxc">${escape_html(item.label)}</span></a>`);
    }
    $$renderer.push(`<!--]-->`);
  }
  $$renderer.push(`<!--]--> <div class="divider my-1 h-px w-8 rounded-full svelte-12buoxc"></div> <button${attr_class("nav-item flex h-11 w-11 items-center justify-center rounded-full transition-all duration-200 svelte-12buoxc", void 0, { "active": moreOpen })} aria-label="Lagi" title="Lagi">`);
  {
    $$renderer.push("<!--[-1-->");
    Grid_2x2($$renderer, { size: 20, strokeWidth: 1.8 });
  }
